...
| `wrapper schema register` | 대화형으로 DB 스키마를 수집하고 Hub에 등록한다. |
| `wrapper schema collect --alias <alias> --lib-dir <app-lib-dir> --jdbc-url <jdbc:dadp:...>` | 스키마 JSON만 생성한다. Hub에는 전송하지 않는다. |
| `wrapper schema register --file <schema-register.json\|-> --alias <alias>` | 기존 스키마 JSON을 Hub에 등록한다. |
| `wrapper refresh [<app-lib-dir>\|--lib-dir <app-lib-dir>]` | Hub에서 Wrapper 옵션과 정책 매핑을 받아 runtime 파일을 갱신한다. |
