...
- 공개 Hub CLI artifact 또는 사내 배포본
- 대상 DB 접속 정보
- Engine URL
- DBA 권한 또는 ACL 반영 권한
- UDF를 소유할 DB 계정

DB UDF 설치에 사용하는 `udf` 명령은 별도 배포물이 아니라 Hub CLI에 포함된 기능이다. 공개 artifact 다운로드 절차는 [Hub CLI Download for DB UDF Operations](db-udf-cli-artifact-download.md)를 따른다.
...
현재 외부 운영자 문서에서는 `dadp udf update`를 기본 절차로 전제하지 않는다. 기존 설치 갱신이 필요하면 먼저 현재 공개 Hub CLI가 해당 명령을 실제로 제공하는지 재검증해야 한다.
...
현재 공개 문서에서는 `--mysql-deploy-plugin`, `--mysql-ssh-user`, `--mysql-ssh-password`를 Hub CLI 설치 기본 옵션으로 전제하지 않는다.
...
- artifact 다운로드와 실제 설치 성공은 다른 단계다. Hub CLI 확보 후 바이너리 자체 점검을 먼저 수행한다.
...
- 외부 운영자 문서에는 현재 공개 Hub CLI에서 확인되지 않은 명령 집합이나 옵션을 넣지 않는다.
