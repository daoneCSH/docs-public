...
배포 전에 Docker Hub에서 사용할 태그를 먼저 확정한다. 공개 문서의 기본 원칙은 "무엇이 최신인지 확인하고, 실제 서버에는 고정 버전을 배포한다"이다.

```bash
curl -L "https://hub.docker.com/v2/repositories/ichito67/dadp-hub/tags?page_size=5"
curl -L "https://hub.docker.com/v2/repositories/ichito67/dadp-engine/tags?page_size=5"
curl -L "https://hub.docker.com/v2/repositories/ichito67/dadp-aggregator/tags?page_size=5"
```

태그 선택 시 다음 원칙을 따른다.

1. 운영 서버에는 `latest` 대신 검증된 고정 태그를 사용한다.
2. Hub와 Engine은 같은 배포 시점에 검증된 조합으로 선택한다.
3. 한쪽만 교체하는 경우에는 정책 동기화와 기본 암복호화 round trip을 다시 확인한다.
...
Amazon Linux 2023에서는 `docker` 패키지만 설치해도 `docker compose`가 바로 제공되지 않을 수 있다. 설치 전에 반드시 다음 명령으로 Compose 사용 가능 여부를 확인한다.

```bash
docker compose version
```

`docker: 'compose' is not a docker command` 오류가 나오면 Docker Compose plugin을 별도로 설치한 뒤 다시 확인한다.
...
특히 Hub 이미지는 `SPRING_DATASOURCE_*`만으로 끝나지 않고 `DB_HOST`, `DB_PORT`, `DB_NAME`, `DB_USERNAME`, `DB_PASSWORD` 값을 함께 맞춰야 할 수 있다. 공개 설치 문서에서는 실제 이미지 동작 기준으로 이 두 계열을 함께 지정하는 방식을 기준선으로 사용한다.
...
Engine은 첫 기동 시점에 Hub가 아직 준비되지 않았으면 오프라인 모드로 올라갈 수 있다. Compose의 `depends_on`은 시작 순서만 제어하며, 애플리케이션 준비 완료까지 보장하지 않는다.

따라서 초기 설치 직후에는 다음 순서로 확인한다.

1. Hub health가 먼저 정상인지 확인한다.
2. Engine 로그에서 Hub 연결 성공 메시지를 확인한다.
3. Engine이 오프라인 모드로 시작했으면 Hub 준비 완료 후 Engine을 다시 시작한다.
...
설치 직후 동기화 상태는 외부 `curl` 하나보다 Engine 로그와 Hub 연결 상태를 함께 본다. 최소 기준은 다음과 같다.

- Hub health가 `200 OK`다.
- Engine health가 `200 OK`다.
- Engine 로그에 Hub 헬스체크 성공 또는 Hub 연결 정상 메시지가 남는다.

`/engine/api/v1/cache/sync-check`는 외부 점검용 기준선으로 사용하지 않는다. 이 경로는 Hub 내부 호출 전제를 가질 수 있으므로 외부에서 직접 호출하면 인증 오류로 오해할 수 있다.
